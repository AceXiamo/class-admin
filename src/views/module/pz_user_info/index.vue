	...
	<el-card>
		...
		<el-table v-loading="state.dataListLoading" :data="state.dataList" border style="width: 100%"
			...
			<el-table-column prop="tags" label="用户自定义标签" header-align="center" align="center">
				<template #default="{ row }">
					<el-tag v-for="(tag,index) in JSON.parse(row.tags||'[]')" :key="tag" closable :disable-transitions="false"
						@close="handleClose(tag,row)" effect="plain" >
						{{ tag }}
					</el-tag>
					<el-input v-if="inputVisible[row.id]" ref="InputRef" v-model="inputValue" class="w-20" size="small"
			...
			<el-table-column label="操作" fixed="right" header-align="center" align="center" width="200">
				<template #default="scope">
					<el-button v-auth="'module:pz_user_info:update'" type="primary" link
...
// 通过申请，将用户状态待验证改为准嘉宾
const userVerifyHandle=(row:any)=>{
	ElMessageBox.confirm('确定通过申请吗?', '提示', {
