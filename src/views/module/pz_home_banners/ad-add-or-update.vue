...
import { usePz_home_bannersAdAddOrUpateApi,usePz_home_bannersAdApi,usePz_home_bannersAdDeleteApi } from '@/api/module/pz_home_banners'
...
const getPz_ad = (id: number) => {
	usePz_home_bannersAdApi(id).then(res => {
		if (res.code === 0) {
...
// 删除
const handleDelete = () => {
	ElMessageBox.confirm('是否确认删除该广告图?', '提示', {
		confirmButtonText: '确定',
		cancelButtonText: '取消',
		type: 'warning'
	}).then(() => {
		usePz_home_bannersAdDeleteApi(dataForm.id).then(res => {
			if (res.code === 0) {
...
const submitHandle = () => {
	dataFormRef.value.validate((valid: boolean) => {
		...
		if (!imageUrl.value) {
			...
			usePz_home_bannersAdAddOrUpateApi(param, dataForm.id).then(res => {
				if (res.code == 0) {
					ElMessage.success({
						message: '上传成功',
						duration: 500,
